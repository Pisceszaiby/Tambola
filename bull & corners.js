...
var corners = [row1[0], row1[4], row3[0], row3[4], row2[2]];
...
$("button").on("click", function () {
    ...
    if (rows.includes(result)) {
        ...
        if (corners.includes(result))
            $(newClass).css("background-color", "#d6299e");
        if (pressed.includes(corners[0]) && pressed.includes(corners[1]) && pressed.includes(corners[2]) && pressed.includes(corners[3]) && pressed.includes(corners[4])) {
            alert("You Have Won")
        }
    }
});
